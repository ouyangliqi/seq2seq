...
class PGN(tf.keras.Model):
    ...
    def __init__(self, params):
        ...
        self.attention = rnn_decoder.BahdanauAttentionCoverage(params[
                                                               "attn_units"])
    ...
    def call(self, enc_output, dec_hidden, enc_inp,
        ...
        your code
        """
        final_dists = calc_final_dist(enc_extended_inp, predictions, attentions, p_gens, batch_oov_len, self.params[
                                      "vocab_size"], self.params["batch_size"])
        # outputs = dict(logits=tf.stack(final_dists, 1), dec_hidden=dec_hidden, attentions=attentions, coverages=coverages)
        if self.params['mode'] == "train":
            outputs = dict(logits=final_dists, dec_hidden=dec_hidden,
                           attentions=attentions, coverages=coverages, p_gens=p_gens)
        else:
